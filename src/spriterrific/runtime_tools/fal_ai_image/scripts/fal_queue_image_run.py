...
def run_image_job(args: argparse.Namespace) -> dict[str, Any]:
    ...
    manifest: dict[str, Any] = {
        ...
        "resolved_arguments": _redact_sensitive(resolved_arguments),
        "preset_defaults": preset.get("defaults", {}),
        "explicit_overrides": _redact_sensitive(overrides),
        ...
        if status_url is not None:
            try:
                ...
                if "405" not in str(exc):
                    raise
                status_response = queue_get_by_url(status_url, api_key, headers=request_headers)
        else:
