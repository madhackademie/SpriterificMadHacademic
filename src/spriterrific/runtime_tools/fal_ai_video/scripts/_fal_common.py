...
def _json_request(method: str, url: str, api_key: str, payload: Any | None = None, headers: dict[str, str] | None = None) -> HttpJsonResponse:
    ...
    try:
        ...
        message = f"fal API error {exc.code}: {error_text}"
...
def infer_extension(url: str, explicit_name: str | None, explicit_content_type: str | None) -> str:
    if explicit_name and "." in explicit_name:
        return Path(explicit_name).suffix or ".bin"
    if explicit_content_type:
        guessed = mimetypes.guess_extension(explicit_content_type.split(";")[0].strip())
        if guessed:
            return guessed
